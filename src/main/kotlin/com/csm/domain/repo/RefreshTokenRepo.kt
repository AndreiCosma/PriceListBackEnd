...
interface RefreshTokenRepo : JpaRepository<RefreshToken, Long> {
    ...
    @Query("select token from RefreshToken token where datediff(day,currentDate,token.creationDate)>=7")
    fun findOldTokens(@Param("currentDate") date: Date): MutableList<RefreshToken>
}